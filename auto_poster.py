...
def create_text_thumbnail(text, filename_prefix):
    try:
        ...
        if not lines: lines = ["2026 부산코리아빌드 (사전등록 시 무료관람)", "핵심 실무 가이드"]
...
def generate_post(campaign, keyword):
    ...
    draft_prompt = f"""당신은 공간 디렉터이자 실내건축 리모델링 감리 전문가입니다.
...
{{
  ...
  "thumb_hook": "{keyword}\\n핵심 실무 가이드 (줄바꿈 \\n)",
  "vibe_keywords": "interior modern house",
...
---METADATA_END---"""
    ...
    vibe_keywords = meta.get('vibe_keywords', 'interior modern house')
